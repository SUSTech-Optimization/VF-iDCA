...
#%%
def iP_DCA(data, DC_Setting = dict(), DEBUG = False):
...
#%%
def iP_DCA2(data, DC_Setting = dict(), DEBUG = False):
    if DEBUG: print("DCA Debuging")
    ...
    class DC_lower:
        ...
        def solve(self, r, iter):
            self.r.value = r 
            result = self.MCG_train.solve(solver = cp.SCS, eps = 1e-2/(np.min([iter+1,100])), warm_start=True)
            # result = self.MCG_train.solve(solver = cp.SCS)
            return result, self.alpha.value, self.beta.value, self.Gamma.value
    ...
    for itr in range(MAX_ITERATION):
        time_0 = Timer()
        obj_lower_k, a_t, b_t, Gamma_t = lower_problem.solve(r, itr)
        time_1 = Timer()
        gamma_r = lower_problem.dual_value()
        # if iter == 0:
            # a, b, Gamma = a_t, b_t, Gamma_t
        approximated_problem.clare_variable_k(a, b, Gamma, r)
        approximated_problem.clare_V_k(gamma_r, obj_lower_k)
        _, a_p, b_p, Gamma_p, r_p = approximated_problem.solve(itr)
        time_2 = Timer()
        r_p = np.maximum(r_p, 0)
        ...
        if DEBUG: print(
            "%3d-th time: %3.1f tr err: %4.2f val err: %4.2f test err: %4.2f " % (itr, dic_for_monitor["time"], dic_for_monitor["train_error"], dic_for_monitor["validation_error"], dic_for_monitor["test_error"]) + 
            "err: %.2e, penalty: %.2e " % (err, penalty) +
            "beta: %3d" % approximated_problem.beta_k.value)

        # Stopping Test
        if err < TOL and penalty < TOL:
            print("%3d-th iteration, pass with %3.1e %3.1e" % (itr, err, penalty))
            break 
